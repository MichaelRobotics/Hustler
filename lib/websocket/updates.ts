...
import { type WebSocketMessage, whopWebSocket } from "./whop-websocket";
...
class RealTimeUpdates {
	...
	private notificationHandlers: Map<
		string,
		(notification: SystemNotification) => void
	> = new Map();
	...
	private setupUpdateHandlers(): void {
		...
		whopWebSocket.subscribe("funnel_updates", (message: WebSocketMessage) => {
			if (message.type === "update") {
				...
				const handler = this.updateHandlers.get(
					`funnel_updates:${update.experienceId}`,
				);
				if (handler) {
					handler(update);
				}
		...
		whopWebSocket.subscribe("resource_updates", (message: WebSocketMessage) => {
			if (message.type === "update") {
				...
				const handler = this.updateHandlers.get(
					`resource_updates:${update.experienceId}`,
				);
				if (handler) {
					handler(update);
				}
