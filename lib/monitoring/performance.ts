...
class PerformanceMonitoring {
	...
	private recordDatabaseMetric(metric: DatabasePerformanceMetrics): void {
		...
		if (this.databaseMetrics.length > this.maxMetricsHistory) {
			this.databaseMetrics = this.databaseMetrics.slice(
	...
	private async collectSystemMetrics(): Promise<PerformanceMetrics> {
		...
		const databaseQueryTime =
			recentDBMetrics.length > 0
				? recentDBMetrics.reduce(
	 ...
	 */
	private checkDatabasePerformanceAlerts(
		metric: DatabasePerformanceMetrics,
	): void {
		if (metric.executionTime > this.alertThresholds.databaseQueryTime) {
			this.sendAlert(
				"performance",
				`Slow database query: ${metric.query.substring(0, 100)}... took ${metric.executionTime}ms`,
				"medium",
			);
		}
	...
	private async checkSystemAlerts(metrics: PerformanceMetrics): Promise<void> {
		if (metrics.memoryUsage > this.alertThresholds.memoryUsage) {
			this.sendAlert(
				"resource",
				`High memory usage: ${metrics.memoryUsage.toFixed(2)}%`,
				"high",
			);
		}
		...
		if (metrics.cpuUsage > this.alertThresholds.cpuUsage) {
			this.sendAlert(
				"resource",
				`High CPU usage: ${metrics.cpuUsage.toFixed(2)}%`,
				"high",
			);
		}
	...
	async getSystemHealthStatus(): Promise<SystemHealthStatus> {
		...
		if (
			latestMetrics.databaseQueryTime > this.alertThresholds.databaseQueryTime
		) {
			alerts.push({
				type: "performance",
	...
	getPerformanceSummary(): {
		...
		const averageDatabaseQueryTime =
			recentDBMetrics.length > 0
				? recentDBMetrics.reduce(
						(sum, metric) => sum + metric.executionTime,
						0,
		...
		const errorCount = this.apiMetrics.filter(
			(metric) => metric.statusCode >= 400,
		).length;
		const errorRate =
	 ...
	 */
	getSlowQueriesReport(limit = 10): DatabasePerformanceMetrics[] {
		return this.databaseMetrics
			.filter(
				(metric) =>
					metric.executionTime > this.alertThresholds.databaseQueryTime,
			)
	 ...
	 * Get error report
	 */
	getErrorReport(limit = 10): APIPerformanceMetrics[] {
