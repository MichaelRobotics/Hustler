...
class ReportingSystem {
	...
	async generateFunnelPerformanceReport(
		...
		return {
			...
			summary: {
				...
				activeFunnels: funnelDetails.filter((f: any) => Number(f.starts) > 0)
					.length,
			...
			funnelDetails: funnelDetails.map((funnel: any) => ({
				...
				completionRate:
					funnel.starts > 0
						? (Number(funnel.completions) / Number(funnel.starts)) * 100
						: 0,
			})),
			topPerformingFunnels: funnelDetails.slice(0, 5).map((funnel: any) => ({
				funnelId: funnel.funnelId,
				funnelName: funnel.funnelName,
				conversions: Number(funnel.conversions) || 0,
				revenue: Number(funnel.revenue) || 0,
	...
	async generateUserEngagementReport(
		...
		const userDetails = await db
			...
			.leftJoin(
				conversations,
				eq(users.experienceId, conversations.experienceId),
			) // This join might need adjustment
			.leftJoin(
				funnelInteractions,
				eq(conversations.id, funnelInteractions.conversationId),
			)
			.where(and(eq(users.experienceId, user.experienceId), ...dateFilter))
		...
		const activeUsers = userDetails.filter(
			(u: any) => Number(u.interactions) > 0,
		).length;
		const totalInteractions = userDetails.reduce(
			(sum: any, u: any) => sum + Number(u.interactions),
			0,
		);
		const totalConversations = userDetails.reduce(
			(sum: any, u: any) => sum + Number(u.conversations),
			0,
		);
		...
		const revenueData = await db
			...
			.where(
				and(
					...
					...(filters.startDate
						? [gte(funnelAnalytics.date, filters.startDate)]
						: []),
					...(filters.endDate
						? [lte(funnelAnalytics.date, filters.endDate)]
						: []),
				),
			);
			...
			revenue: {
				...
				average:
					totals.totalConversions > 0
						? Number(totals.totalRevenue) / Number(totals.totalConversions)
						: 0,
	...
	private generateRecommendations(
		...
		const lowPerformingFunnels = funnelBreakdown.filter(
			(f) => f.starts > 0 && Number(f.conversions) / Number(f.starts) < 0.05,
		);
		...
		if (lowPerformingFunnels.length > 0) {
			recommendations.push({
				...
				action:
					"Review and optimize funnel flows, test different messaging, or consider A/B testing",
			});
		}

		// Check for high-performing funnels
		const highPerformingFunnels = funnelBreakdown.filter(
			(f) => f.starts > 0 && Number(f.conversions) / Number(f.starts) > 0.2,
		);
		...
		if (highPerformingFunnels.length > 0) {
			recommendations.push({
				...
				action:
					"Scale these successful funnels and use them as templates for new campaigns",
			});
		}
		...
		if (Number(totals.totalRevenue) === 0) {
			recommendations.push({
				...
				action:
					"Review funnel setup, check payment integration, and ensure conversion tracking is working",
			});
		}
