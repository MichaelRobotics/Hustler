    ...
    plans.forEach((plan: any) => {
      if (plan.accessPass?.id) {
        ...
        console.log(`⚠️ DEBUGGING: Plan ${plan.id} has no accessPass.id, skipping`);
            ...
            // DEBUGGING: Check if any access passes are missing from plan mapping
            console.log(`🔍 DEBUGGING: Checking for access passes without plans:`);
            accessPasses.forEach((accessPass: any) => {
              const hasPlans = plansByAccessPass.has(accessPass.id);
              const planCount = plansByAccessPass.get(accessPass.id)?.length || 0;
              console.log(`  "${accessPass.title}" (${accessPass.id}): ${hasPlans ? `${planCount} plans` : 'NO PLANS'}`);
            });
