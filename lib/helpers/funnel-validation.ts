...
export const hasValidFlow = (funnel: Funnel): boolean => {
  ...
  if (!funnel.flow.blocks || typeof funnel.flow.blocks !== 'object' || Object.keys(funnel.flow.blocks).length === 0) {
    return false;
  }
  
  // Check if startBlockId exists and is valid
  if (!funnel.flow.startBlockId || typeof funnel.flow.startBlockId !== 'string' || funnel.flow.startBlockId.length === 0) {
    return false;
  }
