...
export function useStateManager(
  ...
  const [state, setState] = useState<StateContext>(() => stateManager.getState());
  ...
  const syncOperations = useMemo(() => state.sync.operations, [state.sync.operations]);
  const isOnline = useMemo(() => state.sync.isOnline, [state.sync.isOnline]);
  const isRealtimeConnected = useMemo(() => state.realtime.isConnected, [state.realtime.isConnected]);
  const realtimeStatus = useMemo(() => state.realtime.connectionStatus, [state.realtime.connectionStatus]);
  const pendingUpdates = useMemo(() => state.realtime.pendingUpdates, [state.realtime.pendingUpdates]);
...
export function useFrontendState(user: AuthenticatedUser | null) {
  const { frontendState, updateFrontendState, dispatch } = useStateManager(user, {
...
export function useSyncOperations(user: AuthenticatedUser | null) {
  ...
  const pendingOperations = useMemo(() => 
    syncOperations.filter(op => op.status === 'pending' || op.status === 'syncing'),
    [syncOperations]
  );
