...
import { whopWebSocket, WebSocketMessage, WebSocketConnection } from '../websocket/whop-websocket';
import { realTimeMessaging, ChatMessage, TypingIndicator, UserPresence } from '../websocket/messaging';
import { realTimeUpdates, FunnelUpdate, ResourceUpdate, SystemNotification, CreditUpdate } from '../websocket/updates';
...
export interface UseWebSocketReturn {
  ...
  // Messaging methods
  sendMessage: (conversationId: string, content: string, type?: 'user' | 'bot' | 'system') => Promise<ChatMessage>;
  sendTypingIndicator: (conversationId: string, isTyping: boolean) => Promise<void>;
  markMessageAsRead: (messageId: string) => Promise<void>;
  
  // Presence methods
  updatePresence: (isOnline: boolean, currentConversationId?: string) => Promise<void>;
...
export function useWebSocket(options: UseWebSocketOptions = {}): UseWebSocketReturn {
  ...
  // Leave channel
  const leaveChannel = useCallback(async (channel: string) => {
  ...
  const sendMessage = useCallback(async (
    ...
    return await realTimeMessaging.sendMessage(userRef.current, conversationId, content, type);
  }, []);
  ...
  const sendTypingIndicator = useCallback(async (conversationId: string, isTyping: boolean) => {
    ...
    await realTimeMessaging.sendTypingIndicator(userRef.current, conversationId, isTyping);
  }, []);
  ...
  // Update presence
  const updatePresence = useCallback(async (isOnline: boolean, currentConversationId?: string) => {
    if (!userRef.current) {
      throw new Error('User not authenticated');
    }
    await realTimeMessaging.updateUserPresence(userRef.current, isOnline, currentConversationId);
  }, []);

  // Subscription methods
  const subscribeToConversation = useCallback((conversationId: string, handler: (message: ChatMessage) => void) => {
    realTimeMessaging.subscribeToConversation(conversationId, handler);
  }, []);

  const subscribeToTyping = useCallback((conversationId: string, handler: (typing: TypingIndicator) => void) => {
  ...
  const subscribeToCreditUpdates = useCallback((userId: string, handler: (update: CreditUpdate) => void) => {
