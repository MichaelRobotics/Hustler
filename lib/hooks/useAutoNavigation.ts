...
export function useAutoNavigation({
  ...
  React.useEffect(() => {
    ...
    if (currentlyGenerating && !wasGenerating) {
      console.log('useAutoNavigation: Generation started, will auto-navigate when complete');
    ...
    if (wasGenerating && !currentlyGenerating && !hasValidFlow(funnel)) {
      console.log('useAutoNavigation: Generation failed, resetting auto-navigation flag');
      setWasGenerating(false);
    }
  }, [isGenerating, funnel.id, funnel.flow, wasGenerating, onNavigate, enabled]);
