...
interface DeleteThemeNotificationProps {
  isOpen: boolean;
  isDeleting: boolean;
  themeName: string;
  onClose: () => void;
...
export const DeleteThemeNotification: React.FC<DeleteThemeNotificationProps> = ({
  isOpen,
  isDeleting,
  themeName,
  onClose,
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Only auto-close if not deleting
      if (!isDeleting) {
        const timer = setTimeout(() => {
          handleClose();
        }, 2000); // Shorter time for deleted state
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 left-1/2 z-[1000] popup-container ${
        ...
        <div className="bg-gradient-to-r from-red-500 to-red-600 rounded-b-xl shadow-2xl border-b-2 border-red-400 p-3 min-w-[280px] max-w-[350px]">
          {/* Header */}
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-sm font-bold text-white">
                {isDeleting ? "Deleting Theme..." : "Theme Deleted!"}
              </h3>
            </div>
            <button
              onClick={handleClose}
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 text-center">
            <div className="text-white text-xs font-medium">
              {isDeleting ? "Please wait..." : themeName}
            </div>
          </div>
        </div>
      </div>
