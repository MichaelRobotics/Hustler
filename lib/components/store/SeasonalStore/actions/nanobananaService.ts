...
// Dynamic import for sharp (server-side only, lazy-loaded when needed)
let sharpModule: any = null;
const getSharp = async () => {
  if (typeof window !== 'undefined') return null;
  if (sharpModule !== null) return sharpModule === false ? null : sharpModule;
  
  try {
    sharpModule = await import('sharp');
    return sharpModule.default || sharpModule;
  } catch (e) {
    console.warn('Sharp not available, image conversion will be limited');
    sharpModule = false; // Cache the failure
    return null;
  }
};
...
REMEMBER: This is BACKGROUND REFINEMENT, not background creation. The background structure must remain exactly the same.`;
    ...
    try {
      ...
      const { uploadBase64ToWhop } = await import('@/lib/utils/whop-image-upload');
  ...
  async generateProductImage(
    ...
    console.log('🎨 [Nano Banana] Image detection details:', {
      ...
      isWhopPlaceholder: originalImageUrl === 'https://assets-2-prod.whop.com/uploads/user_16843562/image/experiences/2025-10-24/e6822e55-e666-43de-aec9-e6e116ea088f.webp',
...
REMEMBER: This is IMAGE REFINEMENT, not product creation. The product must remain exactly the same.`;
    ...
    try {
      ...
      if (hasExistingImage) {
        ...
        if (this.isUnsupportedMimeType(normalizedContentType)) {
          console.log(`🎨 [Nano Banana] Unsupported format (${contentType}) detected - converting to PNG for Gemini compatibility`);
          try {
            const sharp = await getSharp();
            if (sharp) {
              processedBuffer = await sharp(processedBuffer).png().toBuffer() as Buffer<ArrayBuffer>;
              mimeType = 'image/png';
              console.log('🎨 [Nano Banana] Converted image to PNG, new buffer size:', processedBuffer.byteLength, 'bytes');
            } else {
              console.warn('🎨 [Nano Banana] Sharp not available, cannot convert image format');
              hasExistingImage = false;
            }
          } catch (conversionError) {
            console.error('🎨 [Nano Banana] Image conversion failed, falling back to text-to-image generation', conversionError);
            hasExistingImage = false;
          }
        }
...
- Perfect fit for ${targetWidth}x${targetHeight} display dimensions`;
  ...
  private isUnsupportedMimeType(mimeType: string): boolean {
    if (!mimeType) {
      return false;
    }
    const lowered = mimeType.toLowerCase();
    const unsupportedTokens = ['avif', 'jxl', 'heic', 'heif', 'jxr'];
    return unsupportedTokens.some(token => lowered.includes(token));
  }
}
