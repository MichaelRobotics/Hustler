...
export const PerformanceMonitor: React.FC<PerformanceMonitorProps> = ({ 
  ...
  useEffect(() => {
    ...
    if (renderCountRef.current > 10 && timeSinceLastRender < 100) {
      console.warn(`[Performance Warning] ${componentName} is re-rendering too frequently!`);
    }
  ...
  const endTiming = (operation: string) => {
    if (enabled && startTimeRef.current > 0) {
      ...
      if (duration > 16) { // More than one frame at 60fps
        console.warn(`[Performance Warning] ${componentName} - ${operation} took ${duration.toFixed(2)}ms`);
      }
    }
  };
