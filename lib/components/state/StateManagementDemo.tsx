...
import { Card, Text, Heading, Button, Badge, Progress } from 'frosted-ui';
import { 
  Wifi, 
  WifiOff, 
  RefreshCw, 
...
export function StateManagementDemo() {
  const [selectedTab, setSelectedTab] = useState<'overview' | 'realtime' | 'offline' | 'conflicts' | 'sync' | 'optimistic'>('overview');
        ...
        {stats && (
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <Text className="font-mono">{(stats.frontendStateSize / 1024).toFixed(1)} KB</Text>
            </div>
            <div className="flex justify-between">
              <Text>Backend State Size</Text>
              <Text className="font-mono">{(stats.backendStateSize / 1024).toFixed(1)} KB</Text>
            </div>
            <div className="flex justify-between">
        ...
        {stats && (
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <Text className="font-mono">{(stats.cacheHitRate * 100).toFixed(1)}%</Text>
            </div>
            <div className="flex justify-between">
              <Text>Memory Usage</Text>
              <Text className="font-mono">{(stats.memoryUsage / 1024 / 1024).toFixed(1)} MB</Text>
            </div>
            <div className="flex justify-between">
              <Text>Last RefreshCw</Text>
              <Text className="font-mono">
                {stats.lastRefreshCw ? new Date(stats.lastRefreshCw).toLocaleTimeString() : 'Never'}
              </Text>
            </div>
          </div>
        )}
...
function RealtimeTab() {
  const { isConnected, status, pendingUpdates, sync } = useRealtimeStateContext();
  ...
  return (
    <div className="space-y-6">
      <Card className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{pendingUpdates}</div>
            <Text className="text-sm text-gray-600">Pending Updates</Text>
          </div>
          <div className="text-center">
            <Badge variant={isConnected ? 'solid' : 'soft'} className="text-lg px-4 py-2">
              {status}
            </Badge>
            <Text className="text-sm text-gray-600 mt-2">Connection Status</Text>
          </div>
          <div className="text-center">
      ...
      <Card className="p-6">
        <Heading size="3" className="mb-4">Real-time Features</Heading>
...
function OfflineTab() {
  const { isOnline, enableOfflineMode, disableOfflineMode, syncPendingOperations } = useOfflineStateContext();
  ...
  return (
    <div className="space-y-6">
      <Card className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <Badge variant={isOnline ? 'solid' : 'soft'} className="text-lg px-4 py-2">
      ...
      <Card className="p-6">
        <Heading size="3" className="mb-4">Offline Features</Heading>
        <div className="space-y-3">
          <div className="flex items-center space-x-2">
            <CheckCircle className="text-green-500" size={16} />
...
function ConflictsTab() {
  const { conflicts, resolveConflict, hasConflicts } = useConflictStateContext();
        ...
        <div className="text-center py-8">
          {hasConflicts ? (
              ...
              <div className="space-y-3">
                {conflicts.map((conflict, index) => (
                  <div key={index} className="p-4 border border-orange-200 rounded-lg bg-orange-50">
                    <Text className="font-semibold">Entity: {conflict.entityType}</Text>
                    <Text className="text-sm text-gray-600">ID: {conflict.entityId}</Text>
                    <div className="mt-3 space-x-2">
                      <Button size="2" variant="solid">Use Local</Button>
                      <Button size="2" variant="soft">Use Remote</Button>
                      <Button size="2" variant="soft">Merge</Button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-2">
              ...
              <Text className="text-lg font-semibold text-green-600">No Conflicts</Text>
...
function RefreshCwTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      {operations.length > 0 && (
        <Card className="p-6">
          <Heading size="3" className="mb-4">Recent Operations</Heading>
          <div className="space-y-2">
            {operations.slice(0, 10).map((operation, index) => (
              <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                <div>
                  <Text className="font-semibold">{operation.type} {operation.entity}</Text>
                  <Text className="text-sm text-gray-600">ID: {operation.entityId}</Text>
                </div>
                <Badge variant={
                  operation.status === 'completed' ? 'solid' :
                  operation.status === 'failed' ? 'soft' :
                  operation.status === 'syncing' ? 'soft' : 'soft'
                }>
                  {operation.status}
                </Badge>
              </div>
...
function OptimisticTab() {
  const { optimisticUpdates, hasOptimisticUpdates } = useOptimisticStateContext();
      ...
      <Card className="p-6">
        ...
        <div className="text-center py-8">
          {hasOptimisticUpdates ? (
              ...
              <div className="space-y-3">
                {optimisticUpdates.map((update, index) => (
                  <div key={index} className="p-4 border border-purple-200 rounded-lg bg-purple-50">
                    <Text className="font-semibold">{update.type} Operation</Text>
            ...
            <div className="space-y-2">
              <CheckCircle className="text-green-500 mx-auto" size={48} />
              <Text className="text-lg font-semibold text-green-600">No Pending Updates</Text>
              <Text className="text-gray-600">All operations are synchronized</Text>
            </div>
          )}
        </div>
      </Card>
  ...
  return (
    <div className="flex items-center justify-center py-12">
      <div className="text-center space-y-4">
        ...
        <Text className="text-gray-600">Loading state management system...</Text>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center justify-center py-12">
      <div className="text-center space-y-4">
        ...
        <Text className="text-lg font-semibold text-red-600">Error Loading State</Text>
