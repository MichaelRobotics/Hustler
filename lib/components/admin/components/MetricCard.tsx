...
const MetricCard: React.FC<MetricCardProps> = ({
  ...
  return (
    <div className={`group relative bg-gradient-to-br ${colors.bg} p-6 rounded-xl border ${colors.border} ${colors.hoverShadow} transition-all duration-300`}>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className={`flex items-center gap-1 ${trend === 'up' ? 'text-green-600 dark:text-green-400' : 'text-pink-500 dark:text-orange-300'}`}>
          {trend === 'up' ? (
            <ArrowUpRight className="h-4 w-4" strokeWidth={2.5} />
          ) : (
            <ArrowDownRight className="h-4 w-4" strokeWidth={2.5} />
          )}
          <Text size="1" weight="semi-bold">{change}</Text>
        </div>
      </div>
      <div className="mb-2">
