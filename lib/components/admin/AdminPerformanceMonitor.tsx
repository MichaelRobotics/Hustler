...
> = ({ componentName, enabled = process.env.NODE_ENV === "development" }) => {
	...
	useEffect(() => {
		...
		if (renderCountRef.current > 10 && timeSinceLastRender < 100) {
			console.warn(
				`[Admin Performance Warning] ${componentName} is re-rendering too frequently!`,
			);
		}
 ...
 */
export const useAdminPerformanceMonitor = (
	componentName: string,
	enabled = true,
) => {
	...
	const endTiming = (operation: string) => {
		if (enabled && startTimeRef.current > 0) {
			...
			if (duration > 16) {
				...
				console.warn(
					`[Admin Performance Warning] ${componentName} - ${operation} took ${duration.toFixed(2)}ms`,
				);
			}
		}
	};
