  ...
  const handleRender = (
    ...
    if (enabled) {
      console.log(`[Performance] ${id} - ${phase}:`, {
        ...
        efficiency: baseDuration > 0 ? `${((baseDuration - actualDuration) / baseDuration * 100).toFixed(1)}%` : 'N/A'
      });
      ...
      if (actualDuration > 16) { // 60fps threshold
        console.warn(`[Performance Warning] ${id} render took ${actualDuration.toFixed(2)}ms (target: <16ms for 60fps)`);
      }
    }
  };
