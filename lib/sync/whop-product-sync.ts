...
class WhopProductSync {
  ...
  async syncCompanyProducts(
    ...
    try {
      ...
      for (let i = 0; i < whopProducts.length; i += batchSize) {
        ...
        // Update progress
        options.onProgress?.(progress, `Processing batch ${batchNumber}/${totalBatches}`);
        ...
        for (const product of batch) {
          try {
            const syncResult = await this.syncProduct(user, product, options.forceUpdate);
            result.synced++;
  ...
  private async fetchWhopProducts(
    ...
    try {
      ...
      throw new Error(`Failed to fetch products from WHOP: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
