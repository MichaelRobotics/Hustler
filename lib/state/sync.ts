...
class StateSyncManager {
	...
	resolveConflict(conflictId: string, resolution: ConflictResolution): void {
		const conflictIndex = this.state.sync.conflicts.findIndex(
			(c) => c.entityId === conflictId,
		);
	...
	private saveFrontendState(): void {
		try {
			localStorage.setItem(
				"whop-app-frontend-state",
				JSON.stringify(this.state.frontend),
			);
		} catch (error) {
	...
	private detectConflicts(update: StateUpdate): ConflictResolution[] {
		...
		for (const [entityId, optimisticUpdate] of Object.entries(
			this.state.frontend.optimisticUpdates,
		)) {
	...
	private getEntityType(
		actionType: string,
	): "funnel" | "resource" | "conversation" | "message" {
