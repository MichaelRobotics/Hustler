...
class StateValidator {
	...
	sanitizeState(state: any): any {
		...
		// Remove invalid IDs
		if (
			sanitized.selectedFunnelId &&
			typeof sanitized.selectedFunnelId !== "string"
		) {
			sanitized.selectedFunnelId = null;
		}

		if (
			sanitized.selectedConversationId &&
			typeof sanitized.selectedConversationId !== "string"
		) {
			sanitized.selectedConversationId = null;
		}

		if (
			sanitized.selectedResourceId &&
			typeof sanitized.selectedResourceId !== "string"
		) {
		...
		if (Array.isArray(sanitized.conversations)) {
			sanitized.conversations = sanitized.conversations.filter(
				this.isValidConversation,
			);
		}
		...
		if (sanitized.userSettings) {
			sanitized.userSettings = this.sanitizeUserSettings(
	...
	private setupDefaultRules(): void {
		...
		this.rules.set("backend", [
			...
			},
			{
				...
				validator: (value) =>
					["idle", "generating", "completed", "failed"].includes(value),
		...
		this.rules.set("conversation", [
			...
			},
			{
				field: "status",
				validator: (value) =>
					["active", "completed", "abandoned"].includes(value),
	...
	private sanitizeUserSettings(settings: any): any {
		...
		if (
			!sanitized.theme ||
			!["light", "dark", "system"].includes(sanitized.theme)
		) {
		...
		if (
			...
			typeof sanitized.notifications !== "object"
		) {
			sanitized.notifications = {
				email: true,
				push: true,
