...
- Each VALUE_DELIVERY block MUST have "resourceName" field with exact free resource name
- Each OFFER block MUST have "resourceName" field with exact paid product name
...
export const generateFunnelFlow = async (
	...
	const resourceList = resources
		.map((r) => {
			const categoryLabel =
			r.category === "PAID" ? "Paid Product" : "Free Value";
	...
	**VERIFY BEFORE RETURNING:**
	...
	- Every VALUE_DELIVERY/OFFER block has \`"resourceName"\` field with exact resource names
