  ...
  try {
    ...
    const [updatedResource] = await db.update(resources)
      .set({
        ...
        description: input.description !== undefined ? input.description : existingResource.description,
        whopProductId: input.whopProductId !== undefined ? input.whopProductId : existingResource.whopProductId,
        updatedAt: new Date()
      })
...
export async function deleteResource(
  ...
  try {
    ...
    const isUsedInDeployedFunnels = deployedFunnels.some((fr: any) => fr.funnel.isDeployed);
    
    if (isUsedInDeployedFunnels) {
      throw new Error('Cannot delete resource: It is currently used in deployed funnels');
    }
...
export async function syncWhopProducts(
  ...
  try {
    ...
    console.log('WHOP products sync not implemented - SDK products API not available');
    
    return {
      synced: 0,
      created: 0,
      updated: 0,
      errors: ['WHOP products sync not implemented - SDK products API not available']
    };
  } catch (error) {
...
export async function getWhopProducts(
  ...
  try {
    ...
    console.log('WHOP products fetch not implemented - SDK products API not available');
...
export async function bulkDeleteResources(
  ...
  try {
    ...
    if (user.accessLevel === 'customer') {
      const unauthorizedResources = userResources.filter((r: any) => r.userId !== user.id);
    ...
    const usedInDeployedFunnels = deployedFunnels.filter((fr: any) => fr.funnel.isDeployed);
    
    if (usedInDeployedFunnels.length > 0) {
      const resourceNames = usedInDeployedFunnels.map((fr: any) => fr.resourceId);
      throw new Error(`Cannot delete resources: Some are currently used in deployed funnels`);
    }
