...
class AnalyticsSystem {
  ...
  async getFunnelPerformanceMetrics(
    ...
    try {
      ...
      // Calculate rates
      const conversionRate = totals.totalStarts > 0 
        ? (totals.totalConversions / totals.totalStarts) * 100 
        : 0;
      const completionRate = totals.totalStarts > 0 
      ...
      const topBlocks = await db
        .select({
          ...
          conversions: count(sql`CASE WHEN ${funnelInteractions.nextBlockId} IS NULL THEN 1 END`)
        })
        ...
        .where(and(
          ...
          ...(filters.startDate ? [gte(conversations.createdAt, filters.startDate)] : []),
          ...(filters.endDate ? [lte(conversations.createdAt, filters.endDate)] : [])
        ))
      ...
      const topPerformingBlocks = topBlocks.map((block: any) => ({
        ...
        conversionRate: block.interactions > 0 
      ...
      const avgTimeResult = await db
        .select({
          avgTime: avg(sql`EXTRACT(EPOCH FROM (${conversations.updatedAt} - ${conversations.createdAt}))`)
        })
        .from(conversations)
        .where(and(
          eq(conversations.funnelId, funnelId),
    ...
    try {
      ...
      const conversionHistory = await db
        .select({
          ...
          conversions: count(sql`CASE WHEN ${conversations.status} = 'completed' THEN 1 END`),
          revenue: sum(sql`CASE WHEN ${conversations.status} = 'completed' THEN 1 ELSE 0 END * 0`) // Placeholder for revenue
        })
        .from(conversations)
        .where(and(
          eq(conversations.experienceId, user.experienceId),
      ...
      const avgDurationResult = await db
        .select({
          avgDuration: avg(sql`EXTRACT(EPOCH FROM (${conversations.updatedAt} - ${conversations.createdAt}))`)
        })
        .from(conversations)
        .where(and(
          eq(conversations.experienceId, user.experienceId),
  ...
  async getCompanyAnalytics(
    ...
    try {
      ...
      const topPerformingFunnels = topFunnels.map((funnel: any) => ({
        ...
        conversionRate: funnel.starts > 0 
  ...
  private async updateFunnelAnalytics(
    ...
    try {
      ...
      if (existing) {
        ...
        if (updates.starts) updateData.starts = existing.starts + updates.starts;
        if (updates.completions) updateData.completions = existing.completions + updates.completions;
        if (updates.conversions) updateData.conversions = existing.conversions + updates.conversions;
        if (updates.revenue) updateData.revenue = Number(existing.revenue) + updates.revenue;
