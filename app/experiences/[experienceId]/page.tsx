...
export default async function ExperiencePage({
						...
						<div className="space-y-2 text-sm">
							...
							<div className="flex justify-between">
								...
								<span className="text-white">
									{headersList.get("x-whop-user-token") ? "Present" : "Missing"}
								</span>
							</div>
							<div className="flex justify-between">
								<span className="text-gray-400">WHOP Company ID:</span>
								<span className="text-white font-mono">
									{authContext?.user?.experience?.whopCompanyId || "Missing"}
								</span>
							</div>
							<div className="flex justify-between">
								<span className="text-gray-400">Auth Context:</span>
								<span className="text-white">
									{authContext ? "Present" : "Missing"}
								</span>
							</div>
							<div className="flex justify-between">
							...
							{authContext?.user && (
								<>
									...
									<div className="flex justify-between">
										...
										<span className="text-white">
											{authContext.user.credits}
										</span>
									</div>
									<div className="flex justify-between">
										<span className="text-gray-400">Experience ID:</span>
										<span className="text-white font-mono">
											{authContext.user.experienceId}
										</span>
									</div>
									<div className="flex justify-between">
										<span className="text-gray-400">WHOP Company ID:</span>
										<span className="text-white font-mono">
											{authContext.user.experience.whopCompanyId}
										</span>
									</div>
									<div className="flex justify-between">
										<span className="text-gray-400">Experience Name:</span>
										<span className="text-white">
											{authContext.user.experience.name}
										</span>
									</div>
							...
							<div className="flex justify-between">
								...
								<span
									className={`font-semibold ${
										authContext.user.accessLevel === "admin"
											? "text-green-400"
											: authContext.user.accessLevel === "customer"
												? "text-blue-400"
												: "text-red-400"
									}`}
								>
	...
	const accessLevel =
		authContext.user.accessLevel === "no_access"
			? "customer"
			: authContext.user.accessLevel;
