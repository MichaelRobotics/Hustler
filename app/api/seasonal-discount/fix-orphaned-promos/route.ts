...
export const POST = withWhopAuth(async (request: NextRequest, context) => {
  try {
    ...
    const { fixOrphanedPromos } = await import('@/lib/actions/seasonal-discount-actions');
    const result = await fixOrphanedPromos(companyId);

    return NextResponse.json({ success: true, ...result });
  } catch (error) {
    console.error('Error fixing orphaned promos:', error instanceof Error ? error.message : String(error));
    return NextResponse.json(
      { error: 'Failed to fix orphaned promos' },
      { status: 500 }
    );
  }
});
