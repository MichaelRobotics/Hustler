 ...
 */
async function generateBusinessInsightsReportHandler(
	request: NextRequest,
	context: AuthContext,
) {
	try {
		...
		// Generate the report
		const report = await reportingSystem.generateBusinessInsightsReport(
			userContext.user,
			{
				startDate,
				endDate,
		...
		return createSuccessResponse(
			report,
			"Business insights report generated successfully",
		);
	} catch (error) {
