...
async function applyMigration() {
	...
	try {
		...
		if (checkError) {
			console.log(
				"⚠️  Could not check existing columns, proceeding with migration...",
			);
		} else {
			const hasVisualizationState = existingColumns?.some(
				(col) => col.column_name === "visualization_state",
			);
			if (hasVisualizationState) {
				console.log(
					"✅ Column visualization_state already exists, skipping migration",
				);
				return;
			}
		}
    ...
    `;
		...
		if (migrationError) {
			...
			console.log(
				"✅ Can access funnels table, but migration requires database admin privileges",
			);
