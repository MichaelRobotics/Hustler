...
function testWebSocketFileStructure() {
	...
	if (fs.existsSync(whopWebSocketPath)) {
		...
		const hasWhopWebSocketManager = content.includes(
	...
	if (fs.existsSync(messagingPath)) {
		...
		const hasSendTypingIndicator = content.includes(
		...
		const hasSubscribeToConversation = content.includes(
		...
		logTest(
			"Presence management",
			hasUpdateUserPresence && hasMarkMessageAsRead,
		);
	...
	if (fs.existsSync(updatesPath)) {
		...
		const hasSendFunnelGenerationUpdate = content.includes(
			"async sendFunnelGenerationUpdate(",
		);
		const hasSendFunnelDeploymentUpdate = content.includes(
			"async sendFunnelDeploymentUpdate(",
		);
		const hasSendResourceSyncUpdate = content.includes(
			"async sendResourceSyncUpdate(",
		);
		const hasSendAnalyticsUpdate = content.includes(
			"async sendAnalyticsUpdate(",
		);
		const hasSendSystemNotification = content.includes(
...
function testWebSocketHookStructure() {
	...
	if (fs.existsSync(hookPath)) {
		...
		const hasConnectionState = content.includes(
		...
		const hasJoinChannelMethod = content.includes(
			"joinChannel: (channel: string) => Promise<void>",
		);
		const hasSendMessageMethod = content.includes(
			"sendMessage: (conversationId: string, content: string",
		);
		const hasSubscribeMethods =
			content.includes("subscribeToConversation:") &&
		...
		logTest(
			...
			hasConnectionState && hasIsConnected,
		);
...
function testWebSocketTypes() {
	...
	if (fs.existsSync(whopWebSocketPath)) {
		...
		const hasWebSocketConnection = content.includes(
			"interface WebSocketConnection",
		);
		const hasWebSocketConfig = content.includes("interface WebSocketConfig");
		const hasMessageType = content.includes(
		...
		logTest(
			"Message properties",
			hasChannelProperty && hasDataProperty && hasTimestampProperty,
		);
	}
	...
	if (fs.existsSync(messagingPath)) {
		...
		const hasMessageDeliveryStatus = content.includes(
	...
	if (fs.existsSync(updatesPath)) {
		...
		const hasSystemNotification = content.includes(
...
function testWebSocketAPIRoutes() {
	...
	const websocketConnectPath = path.join(
		__dirname,
		"..",
		"app/api/websocket/connect/route.ts",
	);
	const websocketChannelsPath = path.join(
		__dirname,
		"..",
	...
	if (fs.existsSync(websocketChannelsPath)) {
		...
		logTest(
			...
			(hasGET || hasPOST) && hasChannelHandling,
		);
	}
}
...
function testWebSocketSecurity() {
	...
	if (fs.existsSync(whopWebSocketPath)) {
		...
		const hasChannelAccessControl =
			content.includes("channel") && content.includes("access");
