...
function testWebSocketMessageFlow() {
	...
	const whopWebSocketPath = path.join(
		__dirname,
		"..",
	...
	if (fs.existsSync(messagingPath) && fs.existsSync(whopWebSocketPath)) {
		...
		const hasWhopWebSocketImport =
			messagingContent.includes("whop-websocket") ||
		...
		const hasSendMessageUsage = messagingContent.includes(
			"whopWebSocket.sendMessage",
		);
		const hasJoinChannelUsage = messagingContent.includes(
			"whopWebSocket.joinChannel",
		);
		logTest(
			"Messaging uses WebSocket methods",
			hasSendMessageUsage && hasJoinChannelUsage,
		);
		...
		logTest(
			...
			hasBroadcastMessage && hasWebSocketMessage,
		);
	}
}
...
function testRealTimeUpdatesIntegration() {
	...
	if (fs.existsSync(updatesPath) && fs.existsSync(whopWebSocketPath)) {
		...
		const hasWhopWebSocketImport =
			updatesContent.includes("whop-websocket") ||
		...
		const hasSendMessageUsage = updatesContent.includes(
			"whopWebSocket.sendMessage",
		);
		const hasJoinChannelUsage = updatesContent.includes(
			"whopWebSocket.joinChannel",
		);
		logTest(
			"Updates uses WebSocket methods",
			hasSendMessageUsage && hasJoinChannelUsage,
		);
		...
		logTest(
			"Update broadcasting implementation",
			hasBroadcastUpdate && hasWebSocketMessage,
		);
	}
}
...
function testAnalyticsWebSocketIntegration() {
	...
	if (fs.existsSync(analyticsPath) && fs.existsSync(updatesPath)) {
		...
		const hasRealTimeUpdatesImport =
			analyticsContent.includes("realTimeUpdates") ||
		...
		const hasSendAnalyticsUpdate = analyticsContent.includes(
			"sendAnalyticsUpdate",
		);
		const hasSendFunnelUpdate = analyticsContent.includes(
			"sendFunnelGenerationUpdate",
		);
		logTest(
			"Analytics sends real-time updates",
			hasSendAnalyticsUpdate || hasSendFunnelUpdate,
		);
		...
		const hasConversationAnalytics = updatesContent.includes(
			"conversation_analytics",
		);
		logTest(
			"Analytics update types",
			hasFunnelAnalytics && hasRevenueUpdate && hasConversationAnalytics,
		);
	}
}
...
function testFunnelActionsWebSocketIntegration() {
	...
	if (fs.existsSync(funnelActionsPath) && fs.existsSync(updatesPath)) {
		...
		logTest(
			"Funnel actions import real-time updates",
			hasRealTimeUpdatesImport,
		);

		// Check if funnel actions send updates
		const hasSendFunnelGenerationUpdate = funnelContent.includes(
			"sendFunnelGenerationUpdate",
		);
		const hasSendFunnelDeploymentUpdate = funnelContent.includes(
			"sendFunnelDeploymentUpdate",
		);
		logTest(
			"Funnel actions send real-time updates",
		...
		const hasGenerationProgress = updatesContent.includes(
			"generation_progress",
		);
		const hasGenerationCompleted = updatesContent.includes(
...
function testConversationActionsWebSocketIntegration() {
	...
	const conversationActionsPath = path.join(
		__dirname,
		"..",
	...
	if (fs.existsSync(conversationActionsPath) && fs.existsSync(messagingPath)) {
		const conversationContent = fs.readFileSync(
			conversationActionsPath,
			"utf8",
		);
		const messagingContent = fs.readFileSync(messagingPath, "utf8");

		// Check if conversation actions import real-time messaging
		const hasRealTimeMessagingImport =
			conversationContent.includes("realTimeMessaging");
		logTest(
			"Conversation actions import real-time messaging",
			hasRealTimeMessagingImport,
		);

		// Check if conversation actions use messaging
		const hasSendMessage = conversationContent.includes("sendMessage");
		const hasSendTypingIndicator = conversationContent.includes(
			"sendTypingIndicator",
		);
		logTest(
			"Conversation actions use real-time messaging",
			hasSendMessage || hasSendTypingIndicator,
		);
		...
		logTest(
			...
			hasUserMessage && hasBotMessage && hasSystemMessage,
		);
	}
}
...
function testWebSocketHookIntegration() {
	...
	if (
		...
		fs.existsSync(messagingPath) &&
		fs.existsSync(updatesPath)
	) {
		...
		logTest(
			"Hook imports messaging and updates",
			hasMessagingImport && hasUpdatesImport,
		);

		// Check if hook initializes real-time systems
		const hasInitializeMessaging = hookContent.includes(
			"realTimeMessaging.initialize",
		);
		const hasInitializeUpdates = hookContent.includes(
			"realTimeUpdates.initialize",
		);
		logTest(
			"Hook initializes real-time systems",
			hasInitializeMessaging && hasInitializeUpdates,
		);

		// Check hook subscription methods
		const hasSubscribeToConversation = hookContent.includes(
			"subscribeToConversation",
		);
		const hasSubscribeToFunnelUpdates = hookContent.includes(
			"subscribeToFunnelUpdates",
		);
		const hasSubscribeToNotifications = hookContent.includes(
			"subscribeToNotifications",
		);
		logTest(
			"Hook subscription methods",
...
function testWebSocketAPIRoutesIntegration() {
	...
	const channelsRoutePath = path.join(
		__dirname,
		"..",
	...
	if (fs.existsSync(connectRoutePath)) {
		...
		logTest(
			"WebSocket connect route implementation",
			hasWebSocketHandling && hasConnectionHandling,
		);
	}
	...
	if (fs.existsSync(channelsRoutePath)) {
		...
		const hasChannelHandling =
			content.includes("channel") || content.includes("join");
		const hasWebSocketHandling =
			content.includes("websocket") || content.includes("WebSocket");
		logTest(
			"WebSocket channels route implementation",
			hasChannelHandling && hasWebSocketHandling,
		);
	}
}
...
function testWebSocketErrorHandlingIntegration() {
	...
	if (fs.existsSync(whopWebSocketPath)) {
		...
		const hasTimeoutHandling =
			content.includes("timeout") || content.includes("setTimeout");
...
function testWebSocketPerformanceIntegration() {
	...
	if (fs.existsSync(whopWebSocketPath)) {
		...
		const hasExponentialBackoff =
			content.includes("backoff") || content.includes("Math.pow");
...
function testWebSocketSecurityIntegration() {
	...
	if (fs.existsSync(messagingPath)) {
		...
		const hasConversationAccess =
			content.includes("conversation") && content.includes("access");
		const hasCompanyIdCheck = content.includes("companyId");
		logTest(
			"Messaging conversation access control",
			hasConversationAccess && hasCompanyIdCheck,
		);
	}
	...
	if (fs.existsSync(updatesPath)) {
		...
		const hasUserValidation =
			content.includes("user") && content.includes("companyId");
		const hasChannelAccess =
			content.includes("channel") && content.includes("companyId");
...
function testWebSocketDataFlowIntegration() {
	...
	if (fs.existsSync(funnelActionsPath) && fs.existsSync(updatesPath)) {
		...
		const hasFunnelUpdateTrigger =
			funnelContent.includes("sendFunnelGenerationUpdate") ||
		...
		const hasFunnelUpdateHandling =
			updatesContent.includes("FunnelUpdate") ||
	...
	if (fs.existsSync(conversationActionsPath) && fs.existsSync(messagingPath)) {
		...
		const hasMessageTrigger =
			conversationContent.includes("sendMessage") ||
			conversationContent.includes("sendTypingIndicator");
		logTest(
			"Conversation actions trigger WebSocket messages",
			hasMessageTrigger,
		);

		// Check if messaging handles conversation data
		const hasConversationHandling =
			messagingContent.includes("ChatMessage") ||
...
function testWebSocketDocumentationIntegration() {
	...
	if (fs.existsSync(docsPath)) {
		...
		const hasArchitecture =
			content.includes("Architecture") || content.includes("architecture");
