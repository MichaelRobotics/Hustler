...
const mockPerformance = {
  ...
  measure: (name, start, end) => console.log(`⏱️  Performance measure: ${name} - ${end - start}ms`),
};
...
function testRAFThrottling() {
  ...
  const testRAF = () => {
    ...
    if (deltaTime >= targetFrameTime) {
      ...
      if (frameCount <= 10) {
        console.log(`Frame ${frameCount}: ${deltaTime.toFixed(2)}ms (Target: ${targetFrameTime}ms)`);
      }
    }
...
function testKeyboardDetectionOptimization() {
  ...
  keyboardHeights.forEach((height, index) => {
    ...
    console.log(`State ${index + 1}: Height=${currentHeight}px, Keyboard=${isKeyboardVisible ? 'Visible' : 'Hidden'}, Calculated=${keyboardHeight}px`);
  });
