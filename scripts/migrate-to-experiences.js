...
async function migrateToExperiences() {
    ...
    `;
      ...
      `;
		...
		const [userCount] =
			await sql`SELECT COUNT(*) FROM users WHERE experience_id IS NOT NULL`;
		const [funnelCount] =
			await sql`SELECT COUNT(*) FROM funnels WHERE experience_id IS NOT NULL`;
		const [resourceCount] =
			await sql`SELECT COUNT(*) FROM resources WHERE experience_id IS NOT NULL`;
		const [conversationCount] =
			await sql`SELECT COUNT(*) FROM conversations WHERE experience_id IS NOT NULL`;
		const [analyticsCount] =
			await sql`SELECT COUNT(*) FROM funnel_analytics WHERE experience_id IS NOT NULL`;
    ...
    `;
		...
		sampleExperiences.forEach((exp) => {
			...
			console.log(
				`     Users: ${exp.user_count}, Funnels: ${exp.funnel_count}`,
			);
		});
		...
		console.log("\n📝 Next steps:");
		console.log(
			"1. Update your application code to use experienceId instead of companyId",
		);
		console.log("2. Test the application with the new experience-based data");
		console.log(
			"3. Once verified, you can drop the company_id columns and companies table",
		);
	} catch (error) {
