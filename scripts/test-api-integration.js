...
const testStateConsistency = () => {
	...
	states.forEach((state) => {
		...
		console.log(
			`     Frontend: status=${state.frontend.status}, credits=${state.frontend.credits}, anyGenerating=${state.frontend.anyGenerating}`,
		);
		console.log(
			`     Backend: status=${state.backend.status}, credits=${state.backend.credits}`,
		);
