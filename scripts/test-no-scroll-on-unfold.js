...
function testScrollBehaviorChanges() {
  ...
  console.log('    ```tsx');
  console.log('    // Handle keyboard fold/unfold with smooth scroll (only on fold)');
  console.log('    useEffect(() => {');
  console.log('      let previousViewportHeight = window.visualViewport?.height || window.innerHeight;');
  console.log('      ');
  console.log('      const handleViewportChange = () => {');
  console.log('        const currentViewportHeight = window.visualViewport?.height || window.innerHeight;');
  console.log('        ');
  console.log('        // Only scroll when keyboard appears (viewport height decreases)');
...
function testViewportHeightDetection() {
  ...
  console.log('    - `currentViewportHeight < previousViewportHeight` = Keyboard appears');
  console.log('    - `currentViewportHeight > previousViewportHeight` = Keyboard disappears');
  console.log('    - `currentViewportHeight === previousViewportHeight` = No change');
  ...
  console.log('\n  Fallback Behavior:');
  console.log('    - Uses `window.innerHeight` if `visualViewport` not available');
...
function testEdgeCases() {
  ...
  console.log('    - ✅ Modern browsers with visualViewport: Full functionality');
  console.log('    - ✅ Older browsers without visualViewport: Falls back to innerHeight');
  console.log('    - ✅ No breaking changes');
...
function runAllTests() {
  ...
  console.log('6. Observe: Keyboard disappears, NO scroll - maintains position');
  console.log('7. Verify: You can continue reading from where you were');
  console.log('8. Test: Scroll up again, tap input, dismiss - no unwanted scroll');
